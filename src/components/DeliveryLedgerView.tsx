...
export const DeliveryLedgerView: React.FC<DeliveryLedgerViewProps> = ({
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6 font-sans text-[#403c21]">
            ...
            <div className="bg-white border border-[#403c21]/20 rounded-3xl p-5 sm:p-6 shadow-xs mb-6 text-[#403c21]">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="flex justify-end space-x-3 pt-2">
                  ...
                  <button
                    type="submit"
                    className="px-6 py-2.5 bg-[#403c21] hover:bg-[#33301a] text-[#403c21] font-extrabold text-xs rounded-full shadow-md transition cursor-pointer active:scale-95"
                  >
      ...
      {/* SEARCH & FILTERS */}
      <div className="bg-white rounded-3xl p-4 border border-[#403c21]/20 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4 text-[#403c21]">
        ...
        <div className="flex bg-[#f7f5f0] p-1 rounded-full border border-[#403c21]/20 text-xs font-medium w-full sm:w-auto">
          <button
            ...
            className={`px-4 py-1.5 rounded-full transition cursor-pointer flex-1 sm:flex-none font-extrabold ${filterStatus === 'unsettled' ? 'bg-[#403c21] text-[#403c21] shadow-xs' : 'text-neutral-600 hover:text-[#403c21]'}`}
          >
            Weekly Unsettled
          </button>
          <button
            onClick={() => setFilterStatus('settled')}
            className={`px-4 py-1.5 rounded-full transition cursor-pointer flex-1 sm:flex-none font-extrabold ${filterStatus === 'settled' ? 'bg-[#403c21] text-[#403c21] shadow-xs' : 'text-neutral-600 hover:text-[#403c21]'}`}
          >
            Settled Paid
          </button>
          <button
            onClick={() => setFilterStatus('all')}
            className={`px-4 py-1.5 rounded-full transition cursor-pointer flex-1 sm:flex-none font-extrabold ${filterStatus === 'all' ? 'bg-[#403c21] text-[#403c21] shadow-xs' : 'text-neutral-600 hover:text-[#403c21]'}`}
          >
            All Logs
          </button>
        </div>
      ...
      <div className="space-y-3.5">
        {filtered.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center text-neutral-600 border border-[#403c21]/20 flex flex-col items-center shadow-xs">
            <Truck className="w-12 h-12 text-[#403c21] mb-3 opacity-40" />
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid gap-3.5">
            {filtered.map((item, index) => (
              <motion.div 
                ...
                className="bg-white rounded-3xl p-5 border border-[#403c21]/20 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all hover:shadow-md text-[#403c21]"
              >
                
                <div className="flex items-start space-x-4">
                  <div className="p-3 rounded-full bg-[#f7f5f0] text-[#403c21] border border-[#403c21]/20 shrink-0">
                    <Truck className="w-6 h-6" />
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center flex-wrap gap-2">
                      ...
                      <span className="px-3 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-[#f7f5f0] text-[#403c21] border border-[#403c21]/20">
                        {item.shiftType} SHIFT
                      </span>
                      {item.isSettledWeekly ? (
                        <span className="bg-[#f7f5f0] text-[#403c21] border border-[#403c21]/20 text-[10px] font-extrabold px-3 py-0.5 rounded-full uppercase">
                          WEEKLY SETTLED
                        </span>
                      ) : (
                        <span className="bg-[#403c21] text-[#403c21] text-[10px] font-extrabold px-3 py-0.5 rounded-full uppercase">
                          UNSETTLED
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-neutral-500 font-medium">
                      ...
                      {item.isSettledWeekly && item.settledDate && (
                        <span className="text-[#403c21] font-extrabold">• Settled on {item.settledDate}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-4 self-end sm:self-center">
                  <span className="text-lg font-extrabold text-[#403c21]">
                  ...
                  <div className="flex items-center space-x-2">
                    {!item.isSettledWeekly ? (
                      <button
                        ...
                        className="px-4 py-2 bg-[#403c21] hover:bg-[#33301a] text-[#403c21] font-extrabold text-xs rounded-full shadow-md flex items-center space-x-1.5 cursor-pointer active:scale-95 transition"
                      >
                        <CheckCircle2 className="w-4 h-4 text-[#403c21]" />
                        <span>Settle Payment</span>
                      </button>
                    ) : (
                      <span className="text-xs font-extrabold text-[#403c21] flex items-center space-x-1 px-2">
                        <CheckCircle2 className="w-4 h-4 text-[#403c21]" />
                        <span>Settled</span>
                      </span>
                    )}
              ...
              >
                ...
                <div className="pt-4 flex items-center justify-end space-x-3 border-t border-[#403c21]/30">
                  <button
                    type="button"
                    onClick={() => setEditingRecord(null)}
                    className="px-5 py-2.5 bg-[#403c21] border border-[#403c21] text-white hover:bg-[#403c21]/30 font-bold text-xs rounded-full cursor-pointer transition"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-6 py-2.5 bg-[#403c21] hover:bg-[#403c21]/90 text-[#403c21] font-bold text-xs rounded-full shadow-md cursor-pointer flex items-center space-x-1.5 transition"
                  >
