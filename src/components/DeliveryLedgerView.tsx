...
export const DeliveryLedgerView: React.FC<DeliveryLedgerViewProps> = ({
  ...
  const labelClasses = "block text-xs font-bold text-[#0B1D2C]/80 uppercase tracking-wider mb-2";
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6 font-sans text-[#0B1D2C]">
      ...
      {/* HEADER CARD (#0B1D2C Hero Styling with #0B1D2C Accent) */}
      <div className="bg-[#0B1D2C] rounded-3xl p-5 sm:p-6 border border-[#0B1D2C]/40 shadow-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 text-white">
        <div className="flex items-center space-x-4">
          <div className="p-3.5 rounded-full bg-[#0B1D2C] text-white font-bold border border-[#0B1D2C]/40">
        ...
        <div className="flex items-center space-x-4 w-full sm:w-auto">
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-extrabold uppercase tracking-widest text-[#0B1D2C]">
              Unsettled Balance
            </span>
          ...
          <button
            ...
            className="px-5 py-2.5 bg-[#0B1D2C] hover:bg-[#081521] text-white font-black rounded-full shadow-md transition-all flex items-center space-x-2 shrink-0 cursor-pointer text-sm active:scale-95"
          >
      ...
      <AnimatePresence>
        {isFormOpen && (
          ...
          >
            <div className="bg-white border border-[#0B1D2C]/20 rounded-3xl p-5 sm:p-6 shadow-xs mb-6 text-[#0B1D2C]">
              <h3 className="text-base font-extrabold text-[#0B1D2C] mb-5 flex items-center space-x-2">
                <Plus className="w-5 h-5 text-white font-bold" />
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="flex justify-end space-x-3 pt-2">
                  <button
                    ...
                    className="px-5 py-2.5 bg-white border-2 border-[#0B1D2C] text-[#0B1D2C] hover:bg-slate-100 font-extrabold text-xs rounded-full transition cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-6 py-2.5 bg-[#0B1D2C] hover:bg-[#081521] text-white font-black text-xs rounded-full shadow-md transition cursor-pointer active:scale-95"
                  >
      ...
      {/* SEARCH & FILTERS */}
      <div className="bg-white rounded-3xl p-4 border border-[#0B1D2C]/20 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4 text-[#0B1D2C]">
        ...
        <div className="flex bg-[#f7f5f0] p-1 rounded-full border border-[#0B1D2C]/20 text-xs font-medium w-full sm:w-auto">
          <button
            onClick={() => setFilterStatus('unsettled')}
            className={`px-4 py-1.5 rounded-full transition cursor-pointer flex-1 sm:flex-none font-extrabold ${filterStatus === 'unsettled' ? 'bg-[#0B1D2C] text-white font-bold shadow-xs' : 'text-neutral-600 hover:text-[#0B1D2C]'}`}
          >
            Weekly Unsettled
          </button>
          <button
            onClick={() => setFilterStatus('settled')}
            className={`px-4 py-1.5 rounded-full transition cursor-pointer flex-1 sm:flex-none font-extrabold ${filterStatus === 'settled' ? 'bg-[#0B1D2C] text-white font-bold shadow-xs' : 'text-neutral-600 hover:text-[#0B1D2C]'}`}
          >
            Settled Paid
          </button>
          <button
            onClick={() => setFilterStatus('all')}
            className={`px-4 py-1.5 rounded-full transition cursor-pointer flex-1 sm:flex-none font-extrabold ${filterStatus === 'all' ? 'bg-[#0B1D2C] text-white font-bold shadow-xs' : 'text-neutral-600 hover:text-[#0B1D2C]'}`}
          >
            All Logs
          </button>
        </div>
      ...
      <div className="space-y-3.5">
        {filtered.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center text-neutral-600 border border-[#0B1D2C]/20 flex flex-col items-center shadow-xs">
            <Truck className="w-12 h-12 text-[#0B1D2C] mb-3 opacity-40" />
            <h3 className="text-base font-extrabold text-[#0B1D2C]">No delivery account logs</h3>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid gap-3.5">
            {filtered.map((item, index) => (
              <motion.div 
                ...
                className="bg-white rounded-3xl p-5 border border-[#0B1D2C]/20 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all hover:shadow-md text-[#0B1D2C]"
              >
                
                <div className="flex items-start space-x-4">
                  <div className="p-3 rounded-full bg-[#f7f5f0] text-[#0B1D2C] border border-[#0B1D2C]/20 shrink-0">
                    <Truck className="w-6 h-6" />
                  </div>

                  <div className="space-y-1">
                    <div className="flex items-center flex-wrap gap-2">
                      <h4 className="text-base font-extrabold text-[#0B1D2C]">
                        {item.deliveryRiderName}
                      </h4>
                      <span className="px-3 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-[#f7f5f0] text-[#0B1D2C] border border-[#0B1D2C]/20">
                        {item.shiftType} SHIFT
                      </span>
                      {item.isSettledWeekly ? (
                        <span className="bg-[#f7f5f0] text-[#0B1D2C] border border-[#0B1D2C]/20 text-[10px] font-extrabold px-3 py-0.5 rounded-full uppercase">
                          WEEKLY SETTLED
                        </span>
                      ) : (
                        <span className="bg-[#0B1D2C] text-white font-bold text-[10px] font-extrabold px-3 py-0.5 rounded-full uppercase">
                          UNSETTLED
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-neutral-500 font-medium">
                      ...
                      {item.isSettledWeekly && item.settledDate && (
                        <span className="text-[#0B1D2C] font-extrabold">• Settled on {item.settledDate}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-4 self-end sm:self-center">
                  <span className="text-lg font-extrabold text-[#0B1D2C]">
                  ...
                  <div className="flex items-center space-x-2">
                    {!item.isSettledWeekly ? (
                      <button
                        ...
                        className="px-4 py-2 bg-[#0B1D2C] hover:bg-[#081521] text-white font-black text-xs rounded-full shadow-md flex items-center space-x-1.5 cursor-pointer active:scale-95 transition"
                      >
                        <CheckCircle2 className="w-4 h-4 text-[#0B1D2C]" />
                        <span>Settle Payment</span>
                      </button>
                    ) : (
                      <span className="text-xs font-extrabold text-[#0B1D2C] flex items-center space-x-1 px-2">
                        <CheckCircle2 className="w-4 h-4 text-[#0B1D2C]" />
                        <span>Settled</span>
                      </span>
                    )}
                    ...
                    <button
                      ...
                      className="p-2 text-neutral-500 hover:text-[#0B1D2C] hover:bg-slate-100 rounded-full transition cursor-pointer"
                      title="Edit"
                    >
      ...
      <AnimatePresence>
        {editingRecord && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-[#0B1D2C]/80 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          >
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-6 border border-[#0B1D2C]/20 space-y-4 text-[#0B1D2C]"
            >
              <div className="flex items-center justify-between border-b border-[#0B1D2C]/15 pb-4">
                <h3 className="text-base font-extrabold text-[#0B1D2C] flex items-center space-x-2">
                  <Pencil className="w-4 h-4 text-[#0B1D2C]" />
                  <span>Edit Delivery Credit Record</span>
                </h3>
                <button
                  onClick={() => setEditingRecord(null)}
                  className="p-2 text-[#0B1D2C]/70 hover:text-[#0B1D2C] hover:bg-[#f7f5f0] rounded-full transition cursor-pointer"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              >
                ...
                <div className="pt-4 flex items-center justify-end space-x-3 border-t border-[#0B1D2C]/15">
                  <button
                    type="button"
                    onClick={() => setEditingRecord(null)}
                    className="px-5 py-2.5 bg-white border-2 border-[#0B1D2C] text-[#0B1D2C] hover:bg-slate-100 font-extrabold text-xs rounded-full cursor-pointer transition"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-6 py-2.5 bg-[#0B1D2C] hover:bg-[#081521] text-white font-black text-xs rounded-full shadow-md cursor-pointer flex items-center space-x-1.5 transition active:scale-95"
                  >
