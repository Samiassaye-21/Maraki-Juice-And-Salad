  ...
  return (
    <div className="space-y-6 text-[#0B1D2C] font-sans">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-extrabold text-[#0B1D2C]">Other Expenses</h2>
      ...
      {/* Summary Cards (#0B1D2C Hero Style with #0B1D2C Accent) */}
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-[#0B1D2C]/20 rounded-3xl p-5 shadow-xs text-[#0B1D2C]">
          ...
          <p className="text-2xl font-black text-[#0B1D2C] mt-1">{formatCurrency(totalAllTime, currencySymbol)}</p>
          <p className="text-xs font-bold text-[#0B1D2C] mt-0.5">{expenses.length} total records</p>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            ...
            >
              {/* Modal Header */}
              <div className="flex items-center justify-between px-6 py-5 border-b border-[#0B1D2C]/15 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-[#f7f5f0] border border-[#0B1D2C]/20 rounded-full flex items-center justify-center text-[#0B1D2C]">
                    <Wallet className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="font-extrabold text-[#0B1D2C]">New Expense</h3>
                    <p className="text-xs text-neutral-500 font-medium">Record an overhead or bill payment</p>
                  </div>
                </div>
                <button onClick={() => setShowForm(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors cursor-pointer text-[#0B1D2C]/70 hover:text-[#0B1D2C]">
                  <X className="w-5 h-5" />
                </button>
              </div>
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2 p-3 rounded-2xl border text-left transition-all cursor-pointer ${
                            isSelected 
                              ? 'border-[#0B1D2C] bg-[#0B1D2C] text-white font-black shadow-xs' 
                              : 'border-[#0B1D2C]/20 bg-[#f7f5f0] text-[#0B1D2C] hover:bg-slate-100'
                          }`}
                        >
                          <span className="text-xl">{preset.emoji}</span>
                          <span className={`text-sm ${isSelected ? 'font-extrabold text-[#0B1D2C]' : 'font-bold text-[#0B1D2C]'}`}>
                            {preset.label}
                          </span>
                        </button>
                      );
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 border border-[#0B1D2C]/30 bg-white text-[#0B1D2C] font-bold rounded-full text-sm focus:outline-none focus:border-[#0B1D2C] focus:ring-4 focus:ring-[#0B1D2C]/20 placeholder:text-[#0B1D2C]/70 shadow-xs"
                    />
                  </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-10 pr-4 py-2.5 border border-[#0B1D2C]/30 bg-white text-[#0B1D2C] font-bold rounded-full text-sm focus:outline-none focus:border-[#0B1D2C] focus:ring-4 focus:ring-[#0B1D2C]/20 shadow-xs"
                        />
                      </div>
                    </div>
                  </div>
              ...
              {/* Footer */}
              <div className="border-t border-[#0B1D2C]/15 px-6 py-4 flex gap-3 flex-shrink-0">
                <button
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-[#0B1D2C] hover:bg-[#081521] disabled:opacity-40 text-white font-black text-xs rounded-full flex items-center justify-center gap-2 transition-colors cursor-pointer active:scale-95 shadow-md"
                >
      ...
      <div className="space-y-3">
        {sortedExpenses.length === 0 && (
          <div className="text-center py-16 text-neutral-600 bg-white border border-[#0B1D2C]/20 rounded-3xl shadow-xs">
            <Receipt className="w-12 h-12 mx-auto mb-3 text-[#0B1D2C] opacity-40" />
            <p className="font-extrabold text-[#0B1D2C]">No expenses logged yet</p>
        ...
        {sortedExpenses.map(expense => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-[#f7f5f0] border border-[#0B1D2C]/20 rounded-full flex items-center justify-center flex-shrink-0 text-lg">
                    {preset?.emoji || '📦'}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="font-extrabold text-[#0B1D2C]">{formatCurrency(expense.amount, currencySymbol)}</p>
                  </div>
              ...
              <AnimatePresence>
                {isExpanded && (
                  <motion.div
                    ...
                    className="border-t border-[#0B1D2C]/15 overflow-hidden bg-[#f7f5f0]"
                  >
                    <div className="px-5 py-3 flex justify-between items-center">
                      ...
                      <button
                        ...
                        className="text-xs text-neutral-600 hover:text-rose-600 hover:bg-rose-50 flex items-center gap-1 py-1.5 px-3 rounded-full border border-[#0B1D2C]/20 transition-colors cursor-pointer font-bold"
                      >
